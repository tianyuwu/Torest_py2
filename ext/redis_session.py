...
# coding: utf-8
"""
参考 https://github.com/zs1621/tornado-redis-session，并作如下优化
1. 新增set()方法设置单个session，不需要调用save()方法；
2. 配置session默认有效期为1天；
ps:该扩展使用secure_cookie方式存储session_id，即会存一个月，且时间会随着新设置session而刷新
痛点：不能对单个session设置有效期
"""
...
class Session(SessionData):
    ...
    def show(self):
        return self.session_manager.get(self.request_handler)


class SessionManager(object):
    def __init__(self, secret, store_options, session_timeout=3600*24):
        self.secret = secret
